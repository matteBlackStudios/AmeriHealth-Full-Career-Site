...
$cols = Array ("p.JobTitle", "p.JobDescription", "p.PostDate", "p.JobLink", "p.ReqGuid", "p.JobLocation", "p.JobLocationState", "p.JobLocationCity", "p.jobCategory", "p.JobZip");
...
if(!empty($get['o'])){
	...
	$db->orderBy('p.PostDate', 'asc');
}

// Search
if(!empty($get['keywords'])){
    $db->where('(p.JobTitle LIKE "%' . $get['keywords'] . '%" OR p.JobDescription LIKE "%' . $get['keywords'] . '%" OR p.JobLocation LIKE "%' . $get['keywords'] . '%" OR p.JobLocationState LIKE "%' . $get['keywords'] . '%" OR p.JobLocationCity LIKE "%' . $get['keywords'] . '%" OR p.jobCategory LIKE "%' . $get['keywords'] . '%" OR p.ReqGuid LIKE "%' . $get['keywords'] . '%")');
}
...
if(!empty($get['location'])){
    $db->where('p.jobLocation', $get['location']);
}
...
if($count != 0){
    $result['postings'] = '<section id="postings">
						    	<div class="row expanded collapse">
						    		<div id="postings" class="small-12 columns" style="display: block;">
						    			<table style="width: 100%;">
						    			    <thead>
						    					<tr class="text-center">
						    						<th width="200" class="show-for-medium id">JOB ID</th>
						    						<th width="200" class="jobtitle">JOB TITLE</th>
						    						<th width="200" class="location">LOCATION</th>
						    						<th width="200" class="show-for-medium posted">POSTED DATE</th>
						    					</tr>
						    			  	</thead>
						    			  	<tbody>';
    foreach ($postings as $row)
    {
        $result['postings'] .= '<tr onclick="window.document.location=\'http://localhost:8888/amerihealth-full/posting?ReqGuid='. $row['ReqGuid'] .'\'">
